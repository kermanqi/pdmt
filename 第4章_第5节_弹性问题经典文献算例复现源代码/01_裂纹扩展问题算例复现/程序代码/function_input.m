...
m=3;                % 粒子作用范围(包含几层粒子)
length_x=100;    % x方向长度
length_y=40;    % y方向长度；对于一维问题，length_y取值为截面积A
...
boundary_condition_left=[0,0,0];	% 左边界
boundary_condition_right=[0,0,0];	% 右边界
boundary_condition_bottom=[0,2,0]; 	% 下边界
boundary_condition_top=[0,2,0];     % 上边界
...
uniform_load_bottom=[0,-12,0];      % 下边界
uniform_load_top=[0,12,0];      % 上边界
