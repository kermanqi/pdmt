...
if initial_crack==1    % initial_crack=1表示模型中存在初始裂纹
    ...
    if dimension==2
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % 水平裂纹
        if abs(yy1_crack-yy2_crack)<0.01*dettx    % 如果裂纹起始点的y坐标与裂纹结束点的y坐标差值的绝对值小于0.01*dettx，则认定该裂纹为水平裂纹
            for i=1:size_particle %对粒子循环
                ...
                for j=1:size_horizon_particle           % 对第i个粒子域内的粒子进行循环
                    ...
                    if jj>0                             % 如果第i个粒子域内的第j个粒子存在，则执行条件语句中的内容
                        ...
                        if yy_i>yy1_crack && yy_j<yy1_crack  % 如果第i个粒子在裂纹线上侧，第j个粒子在裂纹线下侧
                            detl_x1=abs(xx_j-xx_i);  % detl_x1表示第i个粒子和第jj个粒子在x方向的坐标差
                            if xx_j>xx_i                    % 如果第jj个粒子的x坐标大于第i个粒子的x坐标
                                detl_x2=detl_x1*abs(yy1_crack-yy_i)/abs(yy_j-yy_i); %按case1计算长度detl_x2
                                xx_crack=xx_i+detl_x2;      %由第i个粒子和第jj个粒子组成的键与裂纹线所在直线的交点的x坐标
                            end
                            if xx_j<=xx_i                    % 如果第jj个粒子的x坐标小于等于第i个粒子的x坐标
                                detl_x2=detl_x1*abs(yy1_crack-yy_j)/abs(yy_j-yy_i); %按case2计算长度detl_x2
                                xx_crack=xx_j+detl_x2;      %由第i个粒子和第jj个粒子组成的键与裂纹线所在直线的交点的x坐标
                            end
                            if xx_crack>=xx1_crack && xx_crack<=xx2_crack % 如果键与裂纹线所在直线的交点的x坐标位于裂纹起始点的x坐标与裂纹结束点的x坐标之间，则该键被裂纹线切断
                                damage_particle(i,j)=1;    % damage_particle(i,jj)赋值为1代表由第i个粒子和第jj个粒子组成的键被裂纹线切断
                            end
                        end
                        if yy_i<yy1_crack && yy_j>yy1_crack  % 如果第i个粒子在裂纹线下侧，第j个粒子在裂纹线上侧
                            detl_x1=abs(xx_j-xx_i);  % detl_x1表示第i个粒子和第jj个粒子在x方向的坐标差
                            if xx_j>xx_i                    % 如果第jj个粒子的x坐标大于第i个粒子的x坐标
                                detl_x2=detl_x1*abs(yy1_crack-yy_i)/abs(yy_j-yy_i); %按case3计算长度detl_x2
                                xx_crack=xx_i+detl_x2;      %由第i个粒子和第jj个粒子组成的键与裂纹线所在直线的交点的x坐标
                            end
                            if xx_j<=xx_i                    % 如果第jj个粒子的x坐标小于等第i个粒子的x坐标
                                detl_x2=detl_x1*abs(yy1_crack-yy_j)/abs(yy_j-yy_i); %按case2计算长度detl_x2
                                xx_crack=xx_j+detl_x2;      %由第i个粒子和第jj个粒子组成的键与裂纹线所在直线的交点的x坐标
                            end
                            if xx_crack>=xx1_crack && xx_crack<=xx2_crack % 如果键与裂纹线所在直线的交点的x坐标位于裂纹起始点的x坐标与裂纹结束点的x坐标之间，则该键被裂纹线切断
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % 斜裂纹
        if abs(xx1_crack-xx2_crack)>0.01*dettx && abs(yy1_crack-yy2_crack)>0.01*dettx    % 如果裂纹起始点的x坐标与裂纹结束点的x坐标差值的绝对值大于0.01*dettx且裂纹起始点的y坐标与裂纹结束点的y坐标差值的绝对值大于0.01*dettx，则认定该裂纹为斜裂纹
