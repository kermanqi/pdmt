function[global_stiffness_matrix]=function_global_stiffness_matrix(particle,size_particle,horizon_particle,size_horizon_particle,cf,cm,volume_particle,damage_particle,classify_bond)
	...
	for j=1:size_horizon_particle           % 对第i个粒子域内的粒子进行循环
        if damage_particle(i,j)<0.5         % 如果第i个粒子与第jj个粒子组成的键没有发生破坏，则执行条件语句中的内容
            ...
            if jj>0                             % 如果第i个粒子域内的第j个粒子存在，则执行条件语句中的内容
                ...
                if classify_bond(i,j)>0.5	% classify_bond(i,j)等于1表示纤维方向的键
