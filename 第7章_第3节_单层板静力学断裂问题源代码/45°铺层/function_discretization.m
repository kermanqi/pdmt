function [particle,size_particle,size_x,size_y,size_z,size_displacement_boundary_left,particle_boundary_left,size_displacement_boundary_right,particle_boundary_right, ...
          ...
          boundary_condition_right,boundary_condition_bottom,boundary_condition_top,boundary_condition_behind,boundary_condition_front,xx_min,yy_min,zz_min,xx_max,yy_max,zz_max, ...
          length_x,length_y,length_z,dettx,m)
...
%左边界
    ...
    if boundary_condition_left(1)==1 || boundary_condition_left(2)==1
        for i=1:size_particle
            xx_i=particle(i,1);
            if xx_i<xx_min
                ii_displacement_boundary_left=ii_displacement_boundary_left+1;
                particle_boundary_left(ii_displacement_boundary_left)=i;
            end
        end
        size_displacement_boundary_left=ii_displacement_boundary_left;
    end
%右边界
    ...
    if boundary_condition_right(1)==1 || boundary_condition_right(2)==1
        for i=1:size_particle
            xx_i=particle(i,1);
            if xx_i>xx_max
                ii_displacement_boundary_right=ii_displacement_boundary_right+1;
                particle_boundary_right(ii_displacement_boundary_right)=i;
            end
        end
        size_displacement_boundary_right=ii_displacement_boundary_right;
    end
%下边界
    ...
    if boundary_condition_bottom(1)==1 || boundary_condition_bottom(2)==1
        for i=1:size_particle
            yy_i=particle(i,2);
            if yy_i<yy_min
                ii_displacement_boundary_bottom=ii_displacement_boundary_bottom+1;
                particle_boundary_bottom(ii_displacement_boundary_bottom)=i;
            end
        end
        size_displacement_boundary_bottom=ii_displacement_boundary_bottom;
    end
%上边界
    ...
    if boundary_condition_top(1)==1 || boundary_condition_top(2)==1
        for i=1:size_particle
            yy_i=particle(i,2);
            if yy_i>yy_max
                ii_displacement_boundary_top=ii_displacement_boundary_top+1;
                particle_boundary_top(ii_displacement_boundary_top)=i;
            end
        end
        size_displacement_boundary_top=ii_displacement_boundary_top;
    end
%%%%%%%%%%%%%%%%%%%%%
% 施加均布荷载条件的粒子
%左边界
    ...
    if boundary_condition_left(1)==2 || boundary_condition_left(2)==2
        for i=1:size_particle
            xx_i=particle(i,1);
            if xx_i<(xx_min+dettx)
                ii_uniform_load_left=ii_uniform_load_left+1;
                particle_uniform_load_left(ii_uniform_load_left)=i;
            end
        end
        size_uniform_load_left=ii_uniform_load_left;
    end
%右边界
    ...
    if boundary_condition_right(1)==2 || boundary_condition_right(2)==2
        for i=1:size_particle
            xx_i=particle(i,1);
            if xx_i>(xx_max-dettx)
                ii_uniform_load_right=ii_uniform_load_right+1;
                particle_uniform_load_right(ii_uniform_load_right)=i;
            end
        end
        size_uniform_load_right=ii_uniform_load_right;
    end
%下边界
    ...
    if boundary_condition_bottom(1)==2 || boundary_condition_bottom(2)==2
        for i=1:size_particle
            yy_i=particle(i,2);
            if yy_i<(yy_min+dettx)
                ii_uniform_load_bottom=ii_uniform_load_bottom+1;
                particle_uniform_load_bottom(ii_displacement_boundary_bottom)=i;
            end
        end
        size_uniform_load_bottom=ii_displacement_uniform_load_bottom;
    end
%上边界
    ...
    if boundary_condition_top(1)==2 || boundary_condition_top(2)==2
        for i=1:size_particle
            yy_i=particle(i,2);
            if yy_i>(yy_max-dettx)
                ii_uniform_load_top=ii_uniform_load_top+1;
                particle_uniform_load_top(ii_uniform_load_top)=i;
            end
        end
        size_uniform_load_top=ii_uniform_load_top;
    end
%%%%%%%%%%%%%%%%%%%%%
% 设置为不可发生损伤的粒子
%左边界
...
% 后边界
    ...
    if boundary_condition_bottom(1)==1 || boundary_condition_bottom(2)==1 || boundary_condition_bottom(3)==1
        for i=1:size_particle
            zz_i=particle(i,3);
            if zz_i<zz_min
                ii_displacement_boundary_behind=ii_displacement_boundary_behind+1;
                particle_boundary_behind(ii_displacement_boundary_behind)=i;
            end
        end
        size_displacement_boundary_behind=ii_displacement_boundary_behind;
    end
% 前边界
    ...
    if boundary_condition_front(1)==1 || boundary_condition_front(2)==1 || boundary_condition_front(3)==1
        for i=1:size_particle
            zz_i=particle(i,3);
            if zz_i>zz_max
                ii_displacement_boundary_front=ii_displacement_boundary_front+1;
                particle_boundary_front(ii_displacement_boundary_front)=i;
            end
        end
        size_displacement_boundary_front=ii_displacement_boundary_front;
    end
%%%%%%%%%%%%%%%%%%%%%
% 施加均布荷载条件的粒子
% 左边界
...
% 后边界
    ...
    if boundary_condition_behind(1)==2 || boundary_condition_behind(2)==2 || boundary_condition_behind(3)==2
        for i=1:size_particle
            zz_i=particle(i,3);
            if zz_i<(zz_min+dettx)
                ii_uniform_load_behind=ii_uniform_load_behind+1;
                particle_uniform_load_behind(ii_displacement_boundary_behind)=i;
            end
        end
        size_uniform_load_behind=ii_displacement_uniform_load_behind;
    end
% 前边界
    ...
    if boundary_condition_front(1)==2 || boundary_condition_front(2)==2 || boundary_condition_front(3)==2
        for i=1:size_particle
            zz_i=particle(i,3);
            if zz_i>(zz_max-dettx)
                ii_uniform_load_front=ii_uniform_load_front+1;
                particle_uniform_load_front(ii_uniform_load_front)=i;
            end
        end
        size_uniform_load_front=ii_uniform_load_front;
    end
%%%%%%%%%%%%%%%%%%%%%
% 设置为不可发生损伤的粒子
% 左边界
