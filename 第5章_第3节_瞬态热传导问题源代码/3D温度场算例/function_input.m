...
% 需输入的模型参数
k0=2;          % 导热系数
dimension=3;        % 模型维度 =1 一维问题；=2 二维问题；=3 三维问题
dettx=1.0;          % 相邻粒子的距离
m=3;                % 粒子作用范围(包含几层粒子)
length_x=10;    % x方向长度
length_y=10;    % y方向长度；对于一维问题，length_y取值为截面积A
length_z=20;    % z方向长度；对于二维平面应力问题，length_z取厚度t；对于一维问题，length_z取值为1
xx_min=-5;     % x方向最小坐标
yy_min=-5;     % y方向最小坐标；对于一维问题，length_y取0
zz_min=-10;     % z方向最小坐标；对于一维问题，二维平面应力问题，length_z取0
dt=1.0e-9;              % 每一步的时间增量
time=3.0e-5;              % 计算总时长
output_interval=1000;     % 输出间隔，即每经过output_interval个计算步输出一次结果
density=8.0e-9;              % 材料密度
cv=1.0e2;              % 比热容
temperature_initial=0;              % 初始温度
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
boundary_condition_left=0;	% 左边界
boundary_condition_right=0;	% 右边界
boundary_condition_bottom=0; 	% 下边界
boundary_condition_top=0;     % 上边界
boundary_condition_behind=1;      % 后边界
boundary_condition_front=1;      % 前边界
...
temperature_behind=100;      % 后边界
temperature_front=100;      % 前边界
% 热流边界赋值，如果指定了对应的热流条件，需在下面给出热流数值
heat_flux_left=0;      % 左边界
heat_flux_right=0;      % 右边界
