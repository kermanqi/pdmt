function[global_temperature_vector]=function_temperature_solution(dimension,size_particle,size_temperature_boundary_left,particle_boundary_left_temperature, ...
...
% 施加温度边界条件
    % 左边界
    if boundary_condition_left_temperature==1                                % 如果左边界定义了温度边界条件，则执行条件语句中的内容
        for i=1:size_temperature_boundary_left                     % 对左边界施加温度边界条件的所有粒子循环
            ii=particle_boundary_left_temperature(i,1);                           % ii表示左边界施加温度边界条件的粒子编号
            jj=particle_boundary_left_temperature(i,2);                           % jj表示与左边界施加温度边界条件的粒子关于左边界对称的粒子的编号
            global_temperature0_vector(ii,1)=temperature_left*2-global_temperature0_vector(jj,1);     % 在总体温度矢量的对应位置赋予该时间步的温度
        end
    end
    % 右边界
    if boundary_condition_right_temperature==1                           % 如果右边界定义了温度边界条件，则执行条件语句中的内容
        for i=1:size_temperature_boundary_right                % 对右边界施加温度边界条件的所有粒子循环
            ii=particle_boundary_right_temperature(i,1);                      % ii表示右边界施加温度边界条件的粒子编号
            jj=particle_boundary_right_temperature(i,2);                           % jj表示与右边界施加温度边界条件的粒子关于左边界对称的粒子的编号
...
% 施加热流边界条件
    % 左边界
    if boundary_condition_left_temperature==2    % 如果左边界定义了热流边界条件，则执行条件语句中的内容
    ...
    % 右边界
    if boundary_condition_right_temperature==2    % 如果右边界定义了热流边界条件，则执行条件语句中的内容
    ...
    for i=1:size_particle    % 对所有粒子循环
        ...
        for j=1:size_horizon_particle       % 对第i个粒子作用范围内的所有粒子进行循环
            ...
            if jj>0       % 如果第i个粒子作用范围内的第j个粒子存在，则执行循环条件内的语句
                ...
                thermal_response_vector(i,1)=thermal_response_vector(i,1)+c0_temperature*(temperature_j-temperature_i)/length1_i_jj; % 将粒子jj对粒子i的作用加到热响应矢量中
            end
        end
    end
...
% 施加温度边界条件
    ...
    if boundary_condition_right_temperature==1                           % 如果右边界定义了温度边界条件，则执行条件语句中的内容
        for i=1:size_temperature_boundary_right                % 对右边界施加温度边界条件的所有粒子循环
            ii=particle_boundary_right_temperature(i,1);                      % ii表示右边界施加温度边界条件的粒子编号
            jj=particle_boundary_right_temperature(i,2);                           % jj表示与右边界施加温度边界条件的粒子关于右边界对称的粒子的编号
            global_temperature0_vector(ii,1)=temperature_right*2-global_temperature0_vector(jj,1);     % 在总体温度矢量的对应位置赋予该时间步的温度
        end
    end
    % 下边界
    if boundary_condition_bottom_temperature==1                                % 如果下边界定义了温度边界条件，则执行条件语句中的内容
        for i=1:size_temperature_boundary_bottom                     % 对下边界施加温度边界条件的所有粒子循环
            ii=particle_boundary_bottom_temperature(i,1);                      % ii表示下边界施加温度边界条件的粒子编号
            jj=particle_boundary_bottom_temperature(i,2);                           % jj表示与下边界施加温度边界条件的粒子关于下边界对称的粒子的编号
            global_temperature0_vector(ii,1)=temperature_bottom*2-global_temperature0_vector(jj,1);     % 在总体温度矢量的对应位置赋予该时间步的温度
        end
    end
    % 上边界
    if boundary_condition_top_temperature==1                                % 如果上边界定义了温度边界条件，则执行条件语句中的内容
        for i=1:size_temperature_boundary_top                     % 对上边界施加温度边界条件的所有粒子循环
            ii=particle_boundary_top_temperature(i,1);                      % ii表示上边界施加温度边界条件的粒子编号
            jj=particle_boundary_top_temperature(i,2);                           % jj表示与上边界施加温度边界条件的粒子关于上边界对称的粒子的编号
...
% 施加热流边界条件
    ...
    % 下边界
    if boundary_condition_bottom_temperature==2    % 如果下边界定义了热流边界条件，则执行条件语句中的内容
    ...
    % 上边界
    if boundary_condition_top_temperature==2    % 如果上边界定义了热流边界条件，则执行条件语句中的内容
...
% 施加温度边界条件
    ...
    % 后边界
    if boundary_condition_behind_temperature==1                                % 如果后边界定义了温度边界条件，则执行条件语句中的内容
        for i=1:size_temperature_boundary_behind                   % 对后边界施加温度边界条件的所有粒子循环
            ii=particle_boundary_behind_temperature(i,1);                      % ii表示后边界施加温度边界条件的粒子编号
            jj=particle_boundary_behind_temperature(i,2);                           % jj表示与后边界施加温度边界条件的粒子关于后边界对称的粒子的编号
            global_temperature0_vector(ii,1)=temperature_behind*2-global_temperature0_vector(jj,1);     % 在总体温度矢量的对应位置赋予该时间步的温度
        end
    end
    % 前边界
    if boundary_condition_front_temperature==1                                % 如果前边界定义了温度边界条件，则执行条件语句中的内容
        for i=1:size_temperature_boundary_front                   % 对前边界施加温度边界条件的所有粒子循环
            ii=particle_boundary_front_temperature(i,1);                      % ii表示前边界施加温度边界条件的粒子编号
            jj=particle_boundary_front_temperature(i,2);                           % jj表示与前边界施加温度边界条件的粒子关于前边界对称的粒子的编号
...
% 施加热流边界条件
    ...
    % 后边界
    if boundary_condition_behind_temperature==2    % 如果后边界定义了热流边界条件，则执行条件语句中的内容
    ...
    % 前边界
    if boundary_condition_front_temperature==2    % 如果前边界定义了热流边界条件，则执行条件语句中的内容
