...
%位移求解
    for i=1:size_particle    % 对所有粒子循环
        ...
        uu_i=global_displacement0_vector(i,1);     % 获取当前时刻第i个粒子的x方向位移
        temperature_i=global_temperature_vector(i,1);   % 获取当前时刻第i个粒子的温度
        for j=1:size_horizon_particle       % 对第i个粒子作用范围内的所有粒子进行循环
            ...
            if jj>0       % 如果第i个粒子作用范围内的第j个粒子存在，则执行循环条件内的语句
                ...
                uu_j=global_displacement0_vector(jj,1);     % 获取当前时刻第jj个粒子的x方向位移
                temperature_j=global_temperature_vector(jj,1);% 获取当前时刻第jj个粒子的温度
                ...
                force_density_x=force_density_x+(c0*s0-c0*alf*(temeprature_average-temperature_initial))*(xx_j-xx_i)/length1_i_jj; % 粒子i与粒子jj在x方向的作用加到粒子i所受到的x方向力密度矢量中
            end
        end
        global_accelerate_vector(i,1)=(force_density_x*volume_particle+global_load_density_vector(i,1))/density; % 当前时刻粒子i在x方向的加速度
    end
    ...
    for i=1:size_particle    % 对所有粒子循环
        ...
        for j=1:size_horizon_particle       % 对第i个粒子作用范围内的所有粒子进行循环
            ...
            if jj>0       % 如果第i个粒子作用范围内的第j个粒子存在，则执行循环条件内的语句
                ...
                temeprature_average=(temperature_i+temperature_j)/2;   % 第i个粒子和第jj个粒子的温度取平均值
                force_density_x=force_density_x+(c0*s0-c0*alf*(temeprature_average-temperature_initial))*(xx_j-xx_i)/length1_i_jj; % 粒子i与粒子jj在x方向的作用加到粒子i所受到的x方向力密度矢量中
                force_density_y=force_density_y+(c0*s0-c0*alf*(temeprature_average-temperature_initial))*(yy_j-yy_i)/length1_i_jj; % 粒子i与粒子jj在y方向的作用加到粒子i所受到的y方向力密度矢量中
            end
        end
        global_accelerate_vector(i*2-1,1)=(force_density_x*volume_particle+global_load_density_vector(i*2-1,1))/density; % 当前时刻粒子i在x方向的加速度
        global_accelerate_vector(i*2,1)=(force_density_y*volume_particle+global_load_density_vector(i*2,1))/density; % 当前时刻粒子i在y方向的加速度
    end
    global_velocity_vector=global_velocity0_vector+global_accelerate_vector*dt;          % 更新当前时刻所有粒子的速度
    global_displacement_vector=global_displacement0_vector+global_velocity_vector*dt;          % 更新当前时刻所有粒子的位移
end
    ...
    for i=1:size_particle    % 对所有粒子循环
        ...
        for j=1:size_horizon_particle       % 对第i个粒子作用范围内的所有粒子进行循环
            ...
            if jj>0       % 如果第i个粒子作用范围内的第j个粒子存在，则执行循环条件内的语句
                ...
                ww_j=global_displacement0_vector(jj*3,1);     % 获取当前时刻第jj个粒子的x方向位移
                temperature_j=global_temperature_vector(jj,1);   % 获取当前时刻第jj个粒子的温度
                ...
                force_density_x=force_density_x+(c0*s0-c0*alf*(temeprature_average-temperature_initial))*(xx_j-xx_i)/length1_i_jj; % 粒子i与粒子jj在x方向的作用加到粒子i所受到的x方向力密度矢量中
                force_density_y=force_density_y+(c0*s0-c0*alf*(temeprature_average-temperature_initial))*(yy_j-yy_i)/length1_i_jj; % 粒子i与粒子jj在y方向的作用加到粒子i所受到的y方向力密度矢量中
                force_density_z=force_density_z+(c0*s0-c0*alf*(temeprature_average-temperature_initial))*(zz_j-zz_i)/length1_i_jj; % 粒子i与粒子jj在y方向的作用加到粒子i所受到的z方向力密度矢量中
            end
        end
