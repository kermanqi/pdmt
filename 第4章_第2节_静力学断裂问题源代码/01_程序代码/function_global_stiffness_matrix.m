function[global_stiffness_matrix]=function_global_stiffness_matrix(particle,size_particle,horizon_particle,size_horizon_particle,c0,volume_particle,dimension,damage_particle)
    ...
    for i=1:size_particle                       % 对粒子循环
        ...
        for j=1:size_horizon_particle           % 对第i个粒子域内的粒子进行循环
            ...
            if jj>0                             % 如果第i个粒子域内的第j个粒子存在，则执行条件语句中的内容
                ...
                length_i_jj=abs(xx_j-xx_i);   % 初始构型下粒子i与粒子jj之间的距离为length_i_jj
        ...
        for j=1:size_horizon_particle           % 对第i个粒子域内的粒子进行循环
            if damage_particle(i,j)<0.5         % 如果第i个粒子与第jj个粒子组成的键没有发生破坏，则执行条件语句中的内容
                ...
                if jj>0                             % 如果第i个粒子域内的第j个粒子存在，则执行条件语句中的内容
                    ...
                    length_i_jj=sqrt((xx_j-xx_i)*(xx_j-xx_i)+(yy_j-yy_i)*(yy_j-yy_i)+(zz_j-zz_i)*(zz_j-zz_i));   % 初始构型下粒子i与粒子jj之间的距离为length_i_jj
                    detl_x=(xx_j-xx_i)/length_i_jj;                                     % 计算式子()中的detx
                    detl_y=(yy_j-yy_i)/length_i_jj;                                     % 计算式子()中的dety
