function[damage,max_s0,damage_particle]=function_damage_solution(dimension,particle,size_particle,horizon_particle,size_horizon_particle,global_displacement_vector,damage_particle,sc,itime,output_interval,size_particle_damage,particle_damage)
...
if dimension==3
    for ii=1:size_particle_damage                       % 对所有可以发生损伤的粒子循环
        ...
        uu_i=global_displacement_vector(i*3-2,1);   % 获取第i个粒子的x方向的位移
        vv_i=global_displacement_vector(i*3-1,1);   % 获取第i个粒子的y方向的位移
        ww_i=global_displacement_vector(i*3,1);     % 获取第i个粒子的z方向的位移
        number_damage_bond=0;                       % 用于统计第i个粒子域内发生破坏的键的总数，在此赋初值为0
        number_bond=0;                              % 用于统计第i个粒子域内键的总数，在此赋初值为0
        for j=1:size_horizon_particle           % 对第i个粒子域内的粒子进行循环
        	jj=horizon_particle(i,j);           % 第i个粒子域内的第j个粒子的粒子编号为jj
            if jj>0                             % 如果第i个粒子域内的第j个粒子存在，则执行条件语句中的内容
                ...
                if damage_particle(i,j)<0.5         % 如果第i个粒子与第jj个粒子组成的键没有发生破坏，则执行条件语句中的内容
                    ...
                    uu_j=global_displacement_vector(jj*3-2,1);   % 获取第jj个粒子的x方向的位移
                    vv_j=global_displacement_vector(jj*3-1,1);   % 获取第jj个粒子的y方向的位移
                    ww_j=global_displacement_vector(jj*3,1);     % 获取第jj个粒子的z方向的位移
