function[dimension,dettx,m,length_x,length_y,length_z,xx_min,yy_min,zz_min,xx_max,yy_max,zz_max,detla,c0,volume_particle,size_horizon_particle, ...
    ...
    heat_flux_top,heat_flux_behind,heat_flux_front,body_heat_source,alf,temperature_initial]=function_input()
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                                                             
% 需输入的模型参数
E0=210000;          % 弹性模量
k0=1;          % 导热系数
alf=1e-6;   % 热膨胀系数
temperature_initial=0;    % 模型初始温度
dimension=3;        % 模型维度 =1 一维问题；=2 二维平面应力问题；=3 三维问题
dettx=1;          % 相邻粒子的距离
m=3;                % 粒子作用范围(包含几层粒子)
length_x=30;    % x方向长度
length_y=20;    % y方向长度；对于一维问题，length_y取值为截面积A
length_z=10;    % z方向长度；对于二维平面应力问题，length_z取厚度t；对于一维问题，length_z取值为1
xx_min=-15;     % x方向最小坐标
yy_min=-10;     % y方向最小坐标；对于一维问题，length_y取0
zz_min=-5;     % z方向最小坐标；对于一维问题，二维平面应力问题，length_z取0
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
displacement_left=[0.0,0.0,0.0];      % 左边界
displacement_right=[0,0,0];      % 右边界
...
heat_flux_right=1;      % 右边界
heat_flux_bottom=1;      % 下边界
heat_flux_top=1;      % 上边界
