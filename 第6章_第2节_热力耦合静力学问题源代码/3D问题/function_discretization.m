function [particle,size_particle,size_x,size_y,size_z,size_displacement_boundary_left,particle_boundary_left,size_displacement_boundary_right,particle_boundary_right, ...
    ...
    boundary_condition_bottom_temperature,boundary_condition_top_temperature,boundary_condition_behind_temperature,boundary_condition_front_temperature)
...
%一维情况
if dimension==1    
    size_boundary_x=0;                      % 用来统计x方向施加位移边界条件和温度条件的个数，该变量初始值取0
    size_boundary_left=0;                   % 用于表示左边界是否施加位移条件和温度条件，=0表示左边界未施加位移条件和温度条件，=1表示左边界至少施加了位移条件和温度条件中的一种，该变量初始值取0
    if boundary_condition_left(1)==1 || boundary_condition_left_temperature==1             % 如果左边界施加位移条件或温度条件，则执行条件语句中的内容
        size_boundary_x=size_boundary_x+1;      % x方向施加位移边界条件的个数+1
        size_boundary_left=1;                   % =1 代表左边界施加位移条件
    end
    if boundary_condition_right(1)==1 || boundary_condition_right_temperature==1             % 如果右边界施加位移条件或温度条件，则执行条件语句中的内容
        size_boundary_x=size_boundary_x+1;      % x方向施加位移边界条件的个数+1
    end
...
%左边界
    ...
    if boundary_condition_left(1)==2                                % 如果左边界施加均布荷载条件，则执行条件语句中的内容
        for i=1:size_particle                                       % 对所有粒子进行循环
            xx_i=particle(i,1);                                     % xx_i表示粒子i在x方向的坐标
            if xx_i<(xx_min+dettx) && xx_i>xx_min                   % 如果xx_i小于(x方向最小坐标+dettx)且xx_i大于x方向最小坐标，说明粒子i为位于材料层内最左侧的粒子，则执行条件语句中的内容
...
%右边界
    ...
    if boundary_condition_right(1)==2                               % 如果右边界施加均布荷载条件，则执行条件语句中的内容 
        for i=1:size_particle                                       % 对所有粒子进行循环
            xx_i=particle(i,1);                                     % xx_i表示粒子i在x方向的坐标
            if xx_i>(xx_max-dettx) && xx_i<xx_max                   % 如果xx_i大于(x方向最大坐标-dettx)且xx_i小于x方向最大坐标，说明粒子i为位于材料层内最右侧的粒子，则执行条件语句中的内容
...
if dimension==2
    size_boundary_x=0;                      % 用来统计x方向施加位移边界条件和温度条件的个数，该变量初始值取0
    size_boundary_left=0;                   % 用于表示左边界是否施加位移条件和温度条件，=0表示左边界未施加位移条件和温度条件，=1表示左边界至少施加了位移条件和温度条件中的一种，该变量初始值取0
    if boundary_condition_left(1)==1 || boundary_condition_left(2)==1 || boundary_condition_left_temperature==1               % 如果左边界施加位移条件或温度条件，则执行条件语句中的内容
        size_boundary_x=size_boundary_x+1;      % x方向施加位移边界条件的个数+1
        size_boundary_left=1;                   % =1 代表左边界施加位移条件
    end
    if boundary_condition_right(1)==1 || boundary_condition_right(2)==1 || boundary_condition_right_temperature==1               % 如果右边界施加位移条件或温度条件，则执行条件语句中的内容
        size_boundary_x=size_boundary_x+1;      % x方向施加位移边界条件的个数+1
    end
    size_boundary_y=0;                      % 用来统计y方向施加位移边界条件和温度条件的个数，该变量初始值取0
    size_boundary_bottom=0;                 % 用于表示下边界是否施加位移条件和温度条件，=0表示下边界未施加位移条件和温度条件，=1表示下边界至少施加了位移条件和温度条件中的一种，该变量初始值取0
    if boundary_condition_bottom(1)==1 || boundary_condition_bottom(2)==1 || boundary_condition_bottom_temperature==1         % 如果下边界施加位移条件或温度条件，则执行条件语句中的内容
        size_boundary_y=size_boundary_y+1;  % y方向施加位移边界条件的个数+1
        size_boundary_bottom=1;              % =1 代表下边界施加位移条件
    end
    if boundary_condition_top(1)==1 || boundary_condition_top(2)==1 || boundary_condition_top_temperature==1            % 如果上边界施加位移条件或温度条件，则执行条件语句中的内容
        size_boundary_y=size_boundary_y+1;  % y方向施加位移边界条件的个数+1
    end
...
%下边界
    ...
    if boundary_condition_bottom(1)==2 || boundary_condition_bottom(2)==2 % 如果下边界施加均布荷载条件，则执行条件语句中的内容
        for i=1:size_particle                                             % 对所有粒子进行循环
            yy_i=particle(i,2);                                           % yy_i表示粒子i在y方向的坐标
            if yy_i<(yy_min+dettx) && yy_i>yy_min                         % 如果yy_i小于(y方向最小坐标+dettx)且yy_i大于y方向最小坐标，说明粒子i为位于材料层内最下侧的粒子，则执行条件语句中的内容
...
%上边界
    ...
    if boundary_condition_top(1)==2 || boundary_condition_top(2)==2       % 如果上边界施加均布荷载条件，则执行条件语句中的内容
        for i=1:size_particle                                             % 对所有粒子进行循环
            yy_i=particle(i,2);                                           % yy_i表示粒子i在y方向的坐标
            if yy_i>(yy_max-dettx) && yy_i<yy_max                          % 如果yy_i大于(y方向最大坐标-dettx)且yy_i小于y方向最大坐标，说明粒子i为位于材料层内最上侧的粒子，则执行条件语句中的内容
...
if dimension==3
    size_boundary_x=0;                      % 用来统计x方向施加位移边界条件和温度条件的个数，该变量初始值取0
    size_boundary_left=0;                   % 用于表示左边界是否施加位移条件和温度条件，=0表示左边界未施加位移条件和温度条件，=1表示左边界至少施加了位移条件和温度条件中的一种，该变量初始值取0
    if boundary_condition_left(1)==1 || boundary_condition_left(2)==1 || boundary_condition_left(3)==1 || boundary_condition_left_temperature==1                % 如果左边界施加位移条件或温度条件，则执行条件语句中的内容
        size_boundary_x=size_boundary_x+1;      % x方向施加位移边界条件的个数+1
        size_boundary_left=1;                   % =1 代表左边界施加位移条件
    end
    if boundary_condition_right(1)==1 || boundary_condition_right(2)==1 || boundary_condition_right(3)==1 || boundary_condition_right_temperature==1               % 如果右边界施加位移条件或温度条件，则执行条件语句中的内容
        size_boundary_x=size_boundary_x+1;      % x方向施加位移边界条件的个数+1
    end
    size_boundary_y=0;                      % 用来统计y方向施加位移边界条件和温度条件的个数，该变量初始值取0
    size_boundary_bottom=0;                 % 用于表示下边界是否施加位移条件和温度条件，=0表示下边界未施加位移条件和温度条件，=1表示下边界至少施加了位移条件和温度条件中的一种，该变量初始值取0
    if boundary_condition_bottom(1)==1 || boundary_condition_bottom(2)==1 || boundary_condition_bottom(3)==1 || boundary_condition_bottom_temperature==1         % 如果下边界施加位移条件或温度条件，则执行条件语句中的内容
        size_boundary_y=size_boundary_y+1;  % y方向施加位移边界条件的个数+1
        size_boundary_bottom=1;              % =1 代表下边界施加位移条件
    end
    if boundary_condition_top(1)==1 || boundary_condition_top(2)==1 || boundary_condition_top(3)==1 || boundary_condition_top_temperature==1            % 如果上边界施加位移条件或温度条件，则执行条件语句中的内容
        size_boundary_y=size_boundary_y+1;  % y方向施加位移边界条件的个数+1
    end
    size_boundary_z=0;                      % 用来统计z方向施加位移边界条件和温度条件的个数，该变量初始值取0
    size_boundary_behind=0;                 % 用于表示后边界是否施加位移条件和温度条件，=0表示后边界未施加位移条件和温度条件，=1表示后边界至少施加了位移条件和温度条件中的一种，该变量初始值取0
    if boundary_condition_behind(1)==1 || boundary_condition_behind(2)==1 || boundary_condition_behind(3)==1 || boundary_condition_behind_temperature==1         % 如果后边界施加位移条件或温度条件，则执行条件语句中的内容
        size_boundary_z=size_boundary_z+1;  % y方向施加位移边界条件的个数+1
        size_boundary_behind=1;              % =1 代表后边界施加位移条件
    end
    if boundary_condition_front(1)==1 || boundary_condition_front(2)==1 || boundary_condition_front(3)==1 || boundary_condition_front_temperature==1            % 如果前边界施加位移条件或温度条件，则执行条件语句中的内容
        size_boundary_z=size_boundary_z+1;  % y方向施加位移边界条件的个数+1
    end
    size_x=length_x/dettx+size_boundary_x*m;     % x方向粒子总数=模型x方向长度/每个粒子长度+x方向施加位移边界条件的个数*粒子作用的层数
    size_y=length_y/dettx+size_boundary_y*m;     % y方向粒子总数=模型y方向长度/每个粒子长度+y方向施加位移边界条件的个数*粒子作用的层数
...
% 后边界
    ...
    if boundary_condition_behind(1)==2 || boundary_condition_behind(2)==2 || boundary_condition_behind(3)==2 % 如果后边界施加均布荷载条件，则执行条件语句中的内容
        for i=1:size_particle                                                                                % 对所有粒子进行循环
            zz_i=particle(i,3);                                                                              % zz_i表示粒子i在z方向的坐标
            if zz_i<(zz_min+dettx) && zz_i>zz_min                                                            % 如果zz_i小于(z方向最小坐标+dettx)且zz_i大于z方向最小坐标，说明粒子i为位于材料层内最后侧的粒子，则执行条件语句中的内容
...
% 前边界
    ...
    if boundary_condition_front(1)==2 || boundary_condition_front(2)==2 || boundary_condition_front(3)==2    % 如果前边界施加均布荷载条件，则执行条件语句中的内容 
        for i=1:size_particle                                                                                % 对所有粒子进行循环
            zz_i=particle(i,3);                                                                              % zz_i表示粒子i在z方向的坐标
            if zz_i>(zz_max-dettx) && zz_i<zz_max                                                            % 如果zz_i大于(z方向最大坐标-dettx)且zz_i小于z方向最大坐标，说明粒子i为位于材料层内最前侧的粒子，则执行条件语句中的内容
    ...
    if boundary_condition_left_temperature==1    % 如果左边界施加温度边界条件，则执行条件语句中的内容
        for i=1:size_particle           % 对所有粒子进行循环
            ...
            if xx_i<xx_min              % 如果xx_i小于x方向最小坐标，说明粒子i处于左边界，则执行条件语句中的内容
                ...
                particle_boundary_left_temperature(ii_temperature_boundary_left)=i;        % 在集合particle_boundary_left中记录粒子i的标号
            end
        end
    ...
    if boundary_condition_right_temperature==1    % 如果右边界施加温度边界条件，则执行条件语句中的内容
        for i=1:size_particle           % 对所有粒子进行循环
            ...
            if xx_i>xx_max              % 如果xx_i大于x方向最大坐标，说明粒子i处于右边界，则执行条件语句中的内容
                ...
                particle_boundary_right_temperature(ii_temperature_boundary_right)=i;        % 在集合particle_boundary_right中记录粒子i的标号
            end
        end
    ...
    if boundary_condition_bottom_temperature==1     % 如果下边界施加温度边界条件，则执行条件语句中的内容
        for i=1:size_particle           % 对所有粒子进行循环
            ...
            if yy_i<yy_min              % 如果yy_i小于y方向最小坐标，说明粒子i处于下边界，则执行条件语句中的内容
                ...
                particle_boundary_bottom_temperature(ii_temperature_boundary_bottom)=i;        % 在集合particle_boundary_bottom中记录粒子i的标号
            end
        end
    ...
    if boundary_condition_top_temperature==1     % 如果上边界施加温度边界条件，则执行条件语句中的内容
        for i=1:size_particle           % 对所有粒子进行循环
            ...
            if yy_i>yy_max              % 如果yy_i大于y方向最大坐标，说明粒子i处于上边界，则执行条件语句中的内容
                ...
                particle_boundary_top_temperature(ii_temperature_boundary_top)=i;       % 在集合particle_boundary_top中记录粒子i的标号
            end
        end
...
%上边界
    ...
    if boundary_condition_top_temperature==2                               % 如果上边界施加热流条件，则执行条件语句中的内容
        for i=1:size_particle                                       % 对所有粒子进行循环
            yy_i=particle(i,2);                                     % yy_i表示粒子i在y方向的坐标
            if yy_i>(yy_max-dettx) && yy_i<yy_max                   % 如果yy_i小于(y方向最小坐标+dettx)且yy_i大于y方向最小坐标，说明粒子i为位于材料层内最下侧的粒子，则执行条件语句中的内容
    ...
    if boundary_condition_behind_temperature==1     % 如果后边界施加温度边界条件，则执行条件语句中的内容
        for i=1:size_particle           % 对所有粒子进行循环
            ...
            if zz_i<zz_min              % 如果zz_i小于z方向最小坐标，说明粒子i处于后边界，则执行条件语句中的内容
                ...
                particle_boundary_behind_temperature(ii_temperature_boundary_behind)=i;       % 在集合particle_boundary_behind中记录粒子i的标号
            end
        end
    ...
    if boundary_condition_front_temperature==1     % 如果前边界施加温度边界条件，则执行条件语句中的内容
        for i=1:size_particle           % 对所有粒子进行循环
            ...
            if zz_i>zz_max              % 如果zz_i大于z方向最大坐标，说明粒子i处于前边界，则执行条件语句中的内容
                ...
                particle_boundary_front_temperature(ii_temperature_boundary_front)=i;       % 在集合particle_boundary_front中记录粒子i的标号
            end
        end
