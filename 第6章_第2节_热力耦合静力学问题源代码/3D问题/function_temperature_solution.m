function[global_temperature_vector]=function_temperature_solution(dimension,size_particle,global_heat_conduction_matrix,size_temperature_boundary_left,particle_boundary_left_temperature, ...
         ...
         boundary_condition_top_temperature,boundary_condition_behind_temperature,boundary_condition_front_temperature,volume_particle,dettx)
...
% 施加温度边界条件
    ...
    if boundary_condition_left_temperature==1                                % 如果左边界定义了温度边界条件，则执行条件语句中的内容
        for i=1:size_temperature_boundary_left                     % 对左边界施加温度边界条件的所有粒子循环
            ii=particle_boundary_left_temperature(i);                           % ii表示左边界施加温度边界条件的粒子编号
    ...
    if boundary_condition_right_temperature==1                           % 如果右边界定义了温度边界条件，则执行条件语句中的内容
        for i=1:size_temperature_boundary_right                % 对右边界施加温度边界条件的所有粒子循环
            ii=particle_boundary_right_temperature(i);                      % ii表示右边界施加温度边界条件的粒子编号
            global_heat_conduction_matrix(ii,ii)=global_heat_conduction_matrix(ii,ii)*1.0e8;    % 采用乘大数法对总体热传导矩阵进行操作
...
% 施加温度边界条件
    ...
    if boundary_condition_bottom_temperature==1                                % 如果下边界定义了温度边界条件，则执行条件语句中的内容
        for i=1:size_temperature_boundary_bottom                     % 对下边界施加温度边界条件的所有粒子循环
            ii=particle_boundary_bottom_temperature(i);                           % ii表示下边界施加温度边界条件的粒子编号
            global_heat_conduction_matrix(ii,ii)=global_heat_conduction_matrix(ii,ii)*1.0e8;    % 采用乘大数法对总体热传导矩阵进行操作
    ...
    if boundary_condition_top_temperature==1                                % 如果上边界定义了温度边界条件，则执行条件语句中的内容
        for i=1:size_temperature_boundary_top                     % 对上边界施加温度边界条件的所有粒子循环
            ii=particle_boundary_top_temperature(i);                           % ii表示上边界施加温度边界条件的粒子编号
            global_heat_conduction_matrix(ii,ii)=global_heat_conduction_matrix(ii,ii)*1.0e8;    % 采用乘大数法对总体热传导矩阵进行操作
...
% 施加温度边界条件
    ...
    if boundary_condition_behind_temperature==1                                % 如果后边界定义了温度边界条件，则执行条件语句中的内容
        for i=1:size_temperature_boundary_behind                   % 对后边界施加温度边界条件的所有粒子循环
            ii=particle_boundary_behind_temperature(i);                           % ii表示后边界施加温度边界条件的粒子编号
            global_heat_conduction_matrix(ii,ii)=global_heat_conduction_matrix(ii,ii)*1.0e8;    % 采用乘大数法对总体热传导矩阵进行操作
    ...
    if boundary_condition_front_temperature==1                                % 如果前边界定义了温度边界条件，则执行条件语句中的内容
        for i=1:size_temperature_boundary_front                   % 对前边界施加温度边界条件的所有粒子循环
            ii=particle_boundary_front_temperature(i);                           % ii表示前边界施加温度边界条件的粒子编号
            global_heat_conduction_matrix(ii,ii)=global_heat_conduction_matrix(ii,ii)*1.0e8;    % 采用乘大数法对总体热传导矩阵进行操作
