function[global_displacement_vector]=function_displacement_solution(dimension,size_particle,global_stiffness_matrix,size_displacement_boundary_left,particle_boundary_left, ...
         ...
         boundary_condition_top,boundary_condition_behind,boundary_condition_front,volume_particle,c0,alf,temperature_initial,global_temperature_vector,particle,size_horizon_particle,horizon_particle)
...
% 热荷载
    for i=1:size_particle                       % 对粒子循环
        ...
        temperature_i=global_temperature_vector(i,1);   % 获取第i个粒子的温度
        for j=1:size_horizon_particle           % 对第i个粒子域内的粒子进行循环
            ...
            if jj>0                             % 如果第i个粒子域内的第j个粒子存在，则执行条件语句中的内容
                ...
                temperature_j=global_temperature_vector(jj,1);   % 获取第jj个粒子的温度
                length_i_jj=abs(xx_j-xx_i);     % 粒子i与粒子jj之间的距离为length_i_jj
                temeprature_average=(temperature_i+temperature_j)/2;    % 第i个粒子和第jj个粒子的温度取平均值
                global_load_vector(i,1)=global_load_vector(i,1)-c0*alf*(temeprature_average-temperature_initial)*(xx_j-xx_i)/length_i_jj*volume_particle; % 将温度荷载对第i个粒子的x方向的贡献加入到总体荷载矢量
            end
        end
    end
%%%%%%%%%%%%%%%%%%%%%%%
%施加体力
    if body_force(1)~=0    % 如果x方向体力密度不为0，说明施加了x方向的体力荷载，则执行条件语句中的内容
        for i=1:size_particle % 对所有粒子循环
            global_load_vector(i,1)=global_load_vector(i,1)+body_force(1)*volume_particle; % 在总体荷载向量对应位置增加荷载
        end
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for i=1:size_particle                       % 对粒子循环
        ...
        for j=1:size_horizon_particle           % 对第i个粒子域内的粒子进行循环
            ...
            if jj>0                             % 如果第i个粒子域内的第j个粒子存在，则执行条件语句中的内容
                ...
                temeprature_average=(temperature_i+temperature_j)/2;    % 第i个粒子和第jj个粒子的温度取平均值
                global_load_vector(i*2-1,1)=global_load_vector(i*2-1,1)-c0*alf*(temeprature_average-temperature_initial)*detl_x*volume_particle; % 将温度荷载对第i个粒子的x方向的贡献加入到总体荷载矢量
                global_load_vector(i*2,1)=global_load_vector(i*2,1)-c0*alf*(temeprature_average-temperature_initial)*detl_y*volume_particle; % 将温度荷载对第i个粒子的y方向的贡献加入到总体荷载矢量
            end
        end
    end
%%%%%%%%%%%%%%%%%%%%%%%
%施加体力
    if body_force(1)~=0    % 如果x方向体力密度不为0，说明施加了x方向的体力荷载，则执行条件语句中的内容
        for i=1:size_particle    % 对所有粒子循环
            global_load_vector(i*2-1,1)=global_load_vector(i*2-1,1)+body_force(1)*volume_particle;    % 在总体荷载向量对应位置增加荷载
        end
    end
    if body_force(2)~=0    % 如果y方向体力密度不为0，说明施加了x方向的体力荷载，则执行条件语句中的内容
        for i=1:size_particle    % 对所有粒子循环
            global_load_vector(i*2,1)=global_load_vector(i*2,1)+body_force(2)*volume_particle;    % 在总体荷载向量对应位置增加荷载
        end
    end
%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for i=1:size_particle                       % 对粒子循环
        ...
        for j=1:size_horizon_particle           % 对第i个粒子域内的粒子进行循环
            ...
            if jj>0                             % 如果第i个粒子域内的第j个粒子存在，则执行条件语句中的内容
                ...
                temeprature_average=(temperature_i+temperature_j)/2;    % 第i个粒子和第jj个粒子的温度取平均值
                global_load_vector(i*3-2,1)=global_load_vector(i*3-2,1)-c0*alf*(temeprature_average-temperature_initial)*detl_x*volume_particle; % 将温度荷载对第i个粒子的x方向的贡献加入到总体荷载矢量
                global_load_vector(i*3-1,1)=global_load_vector(i*3-1,1)-c0*alf*(temeprature_average-temperature_initial)*detl_y*volume_particle; % 将温度荷载对第i个粒子的y方向的贡献加入到总体荷载矢量
                global_load_vector(i*3,1)=global_load_vector(i*3,1)-c0*alf*(temeprature_average-temperature_initial)*detl_z*volume_particle; % 将温度荷载对第i个粒子的z方向的贡献加入到总体荷载矢量
            end
        end
    end
%%%%%%%%%%%%%%%%%%%%%%%
%施加体力
    if body_force(1)~=0    % 如果x方向体力密度不为0，说明施加了x方向的体力荷载，则执行条件语句中的内容
        for i=1:size_particle     % 对所有粒子循环
            global_load_vector(i*3-2,1)=global_load_vector(i*3-2,1)+body_force(1)*volume_particle;    % 在总体荷载向量对应位置增加荷载
        end
    end
    if body_force(2)~=0    % 如果y方向体力密度不为0，说明施加了x方向的体力荷载，则执行条件语句中的内容
        for i=1:size_particle    % 对所有粒子循环
            global_load_vector(i*3-1,1)=global_load_vector(i*3-1,1)+body_force(2)*volume_particle;    % 在总体荷载向量对应位置增加荷载
        end
    end
    if body_force(3)~=0    % 如果z方向体力密度不为0，说明施加了x方向的体力荷载，则执行条件语句中的内容
        for i=1:size_particle    % 对所有粒子循环
            global_load_vector(i*3,1)=global_load_vector(i*3,1)+body_force(3)*volume_particle;     % 在总体荷载向量对应位置增加荷载
        end
    end
%%%%%%%%%%%%%%%%%%%%%%%
end
